...
type FriendMyListProps = {
  hangouts: Hangout[];
  schedules: Schedule[];
};

export const FriendMylist: FC<FriendMyListProps> = ({ hangouts, schedules }) => {
  const hangoutList = [
  ...
  return (
    <div className="bg-new-white shadow-lg rounded-lg p-4 mt-32">
      {hangouts !== null && hangouts.length !== 0 ? (
        ...
        <div className="py-6 border-b border-new-blue-900">
          <Text style="text-center ">遊びは登録されていません</Text>
        </div>
      )}
      {schedules !== null && schedules.length !== 0 ? (
        ...
        <div className="py-6">
          <Text style="text-center">予定は登録されていません</Text>
        </div>
      )}
    </div>
  );
